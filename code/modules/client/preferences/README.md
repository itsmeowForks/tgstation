...
There are some helper procs you can use as well. `render_preview_outfit(outfit_type)` will take an outfit and give you an icon of someone wearing those clothes. `finish_preview_outfit` will, given a /datum/universal_icon, resize it appropriately and zoom in on the head. Note that this will look bad on anything that isn't a human, so if you have a non-human antagonist (such as sentient disease), just run `icon.scale(ANTAGONIST_PREVIEW_ICON_SIZE, ANTAGONIST_PREVIEW_ICON_SIZE)`.
...
/datum/antagonist/changeling/get_preview_icon()
	...
	final_icon.shift(WEST, ICON_SIZE_X / 2, 32, 32)
	final_icon.shift(EAST, ICON_SIZE_X / 2, 32, 32)

	split_icon.shift(EAST, ICON_SIZE_X / 2, 32, 32)
	split_icon.shift(WEST, ICON_SIZE_X / 2, 32, 32)
